...
{Notice that the constant equation $2x + 3y - z - 9 = 0$ is the equation for a plane.  This is not a closed and bounded region, so we can't guarantee that $f$ attains both a maximum and minimum on the constraint.  In fact, a little thought should convince us that $f$ does not have a maximum value.  Notice that $f$ increases as the point $(x,y,z)$ gets farther from the origin.  Since we can find points on the plane $2x+3y-z-9=0$ that are arbitrarily far from the origin, $f$ can get arbitrarily large on the constraint.
